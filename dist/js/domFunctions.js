...
const createCurrentConditionsDivs = (weatherObj, unit) => {
    ...
    const wind = createElem("div", "wind", `${Math.round(Number(weatherObj.current.wind_speed))} ${windUnit}`);
...
const createDailyForecastDivs = (dayWeather) => {
    ...
    const dayHigh = createElem("div", "dayHigh", `${Math.round(Number(dayWeather.temp.max))}°`);
    const dayLow = createElem("div", "dayLow", `${Math.round(Number(dayWeather.temp.min))}°`);
